...
def write_empty_leaf(out_dir: Path, arm_len_ref_m: float):
    safe_mkdir(out_dir)
...
def main():
    ...
    # Placeholder template project: create standard structure for each input video.
    for video in videos:
        model_name = cfg.get("model", {}).get("pose3d_name", "human_pose_3d_lifter")
        leaf = out_root / video.stem / model_name / "VIDEO" / args.device.upper()
        write_empty_leaf(leaf, args.arm_len_ref_m)
    ...
    print("mmpose_3d template project prepared.")
    print(f"Videos directory: {videos_dir}")
    print(f"Runs directory:   {out_root}")
    print("Next step: replace placeholder inference in scripts/benchmark.py with actual MMPose 2D+3D pipeline.")
